...
- `Call` käskyt kutsuvat useaan otteeseen muita funktiota. `puts@plt` tulostaa kysymyksen, sitten ohjelma lukee syötteen `__isoc99_scanf@plt`, tämän jälkeen ohjelma kutsuu `mAsdf3a` funktiota. Kohdassa `main+153` määrittyy se meneekö `eax` paluuarvo onnistumispolkuun, jolloin flägi tulisi esiin.
...
- Tässä kohdassa hyödynsin tekoälyä, jotta ymmärtäisin funktion toimintaa. Laitoin tekoälylle kuvakaappauksen funktion sisällöstä ja pyysin tätä auttamaaan selittämään funktion tarkoitus

<img width="604" height="649" alt="image" src="https://github.com/user-attachments/assets/2810eea3-4083-4a66-ae37-b9f730b5ce44" />

- Funktio tekee merkkijonolle pituustarkistuksen `strlen` ja vertailee niitä `cmp r12d, edx`n tässä vaiheessa jos pituus ei täsmää niin tulos on fail
- Sen jälkeen loopissa tapahtuu parillisuuden tarkistus `test $0x1, %al`, jossa parillinen indeksiin lisätään 3 (add $0x3) ja parittomat indeksit vähentyvät 7:llä (sub $0x7). Testin jälkeen huomataan kohta `je` eli parillinen arvo jatkaa "jump if equal" kohtaan, osoitin hyppää suoraan `mAsdf3a+63`, jossa tapahtuu `add` käsky eli 3 lisäys.
- Ohjelman `test $1, al` kohdassa tapahtuu AND-operaatio,  missä molempien X-Y arvojen tulee olla 1, jotta luku on pariton ja 0, jotta luku on parillinen. 1 & 1 = 1, 1 & 0 = 0 jne...

- Palataan main funktion kohtaan ja katsotaan missä vaiheessa salasanantarkistus funktio kutsutaan ensimmäisen kerran

<img width="580" height="25" alt="image" src="https://github.com/user-attachments/assets/1582e183-c8ac-4385-b1fd-7e6e44042d55" />

- Rivi 123. Asetetaan breakpoint siihen ja tutkitaan tarkemmin sitä

<img width="672" height="349" alt="image" src="https://github.com/user-attachments/assets/21857d1b-467e-4fc0-a11d-6da9240c92e2" />

- Komennolla `x/s` voidaan nähdä muisti stringinä, kokeillaan niitä
- Saadaan merkkijono `anLTj4u8`
- Kun merkkijono asetetaan ASCII-taulukkoon niin, että näkyvät desimaaleina voidaan selvittää uusi merkkijono, kun käytetään tarkistusfunktion logiikkaa. Parillinen indeksi i+3, pariton i-7. Hyödynsin tässä tekoälyä.

<img width="85" height="356" alt="image" src="https://github.com/user-attachments/assets/bbd75e44-bdce-4205-a75a-40a7190c5ebb" />

<img width="313" height="279" alt="image" src="https://github.com/user-attachments/assets/ff1c0c6d-147d-4d29-adc8-b4dd18d864fa" />


SALASANA: `dg0Mm-x1`

<img width="625" height="99" alt="image" src="https://github.com/user-attachments/assets/9eb7c122-1631-4ab1-b5aa-cbde5e8ca6ff" />
...
- Blokki `0x1197` on string lenght eli pituustarkistus. Syöte hylätään, jos syötteen pituus ei ole tasan 6. Jos pituustarkistus onnistuu se jatkaa edelleen `check_pw` tarkistusblokkiin.

<img width="322" height="394" alt="image" src="https://github.com/user-attachments/assets/d2073ed8-a02a-4341-a80d-6c2d87f0d538" />

- Toinen tarkistusblokki on `0x11c9`, joka on check_pw eli sisältötarkistus. `test eax, eax` tämä tarkistaa funktion paluuarvon, onko 0 vai ei. `eax !=0` niin se siirtyy onnistumispolkuun, jos taas `eax == 0` se tulostaa fail-polun "No..."

<img width="225" height="197" alt="image" src="https://github.com/user-attachments/assets/65352380-c623-46d1-b6f9-bb793a942a49" />

- Voidaan päätellä, jos pituustarkistus onnistuu se hyppää suoraan sisältötarkistukseen, jos taas sisältötarkistus epäonnistuu se hyppää suoraan virhetulostukseen.

- Avataan disassembly näkymä radare2 debuggerissa 'pdf @ sym.check_pw'
- Nähdään eri argumentit, joista rdi on syöte, rsi on merkkijono lAmBdA ja rdx jonkintyylinen maski salasanalle
- Disassemblystä nähdään tarkemmin, että funktio käy läpi käyttäjän syötettä ja tekee tarkastuksen vastaako se oikeita arvoja

<img width="957" height="532" alt="image" src="https://github.com/user-attachments/assets/0daaf3a9-32b8-4ee4-a3e0-d71a4abaee68" />


- Salasana on 6 merkkinen merkkijono, joten otetaan kuusi ensimmäistä tavua eli `02, 03, 02, 03, 05, 00` sain tämän tulostettua komennolla 'px 8 @ rdi'
...
- Loopin toiminta perustuu siihen, että se vertailee merkkejä ja laskee samalla summaa, sekä pituutta. Ohjelma vertaa laskurin pituutta pituuteen `0x10` ja summaa arvoon 0x6e2 eli desimaaleina se on juuri 1762. Looppaus käy merkkijonoa läpi kunnes se saavuttaa `\0`
...
# Lähteet:
- RayNull, 2025: 5 Ways to cause Segmentation Fault in C (and how to fix) https://www.youtube.com/watch?v=1RrGVHF97Gs
- CyberCheff: https://gchq.github.io/CyberChef/#recipe=From_Hex('Auto')&input=NmUgNDQgNmYgNDUgNjkgNDE
- Lab2 tehtävässä: OpenAI ChatGPT 5.2 "Auta minua selvittämään ohjelman toiminta" (kuvakaappaus ohjelmasta), ja salasanan käännös ASCII-taulukon avulla.
- NoraCodes GitHub CrackMes: https://github.com/NoraCodes/crackmes
- Crackme tehtävissä hyödynnetty: https://nora.codes/tutorial/an-intro-to-x86_64-reverse-engineering/
- Hammond 2023: https://www.youtube.com/watch?v=Tmdnsre9z7s&t=512s
